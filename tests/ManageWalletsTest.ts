...
class ManageWalletsTest extends BaseTestSuite {
    ...
    public async hasArketypeInDescription() {
        await this.assert.equal(await this.browser.findElement(By.css('.whitelabel-page__info')).getAttribute('innerHTML'), '<span>Please select your wallets <strong>Arketype</strong> is allowed to access.</span>');
    }
    ...
    public async hasTwoActiveWalletsThatAreSelectable() {
        ...
        await this.assert.equal(await walletCards[0].getAttribute('class'), 'wallet-card', 'Wallet Card deselected');
        await walletCards[0].click();
        await this.assert.equal(await walletCards[0].getAttribute('class'), 'wallet-card wallet-card--active', 'Wallet Card selected');
    }
    ...
    public async canOpenAndCloseNewWalletWindow() {
        ...
        await this.assert.equal(await newWalletLink.getAttribute('innerHTML'), 'New Wallet', 'New Wallet link present.');
        await newWalletLink.click();

        const modal: WebElement = await this.browser.findElement(By.css('.modal-mask'));
        await this.assert.equal(await modal.findElement(By.css('.modal__title')).getAttribute('innerHTML'), 'Confirmation Required', 'Modal opened after new wallet clicked.');

        const modalConfirmButton: WebElement = await modal.findElement(By.css('button[disabled="disabled"]'));
        await this.assert.equal(await modalConfirmButton.getAttribute('innerHTML'), 'Confirm', 'Modal confirm button disabled with no pin entered');

        const pincode: WebElement = await modal.findElement(By.css('.control__input.control__input--pincode'));
        await pincode.sendKeys('8473');
        await this.assert.equal(await modalConfirmButton.getAttribute('disabled'), null, 'Modal confirm button active after entering pin');
        ...
        await this.assert.isTrue(await this.browser.wait(until.stalenessOf(modal), 750), 'Modal cancel clicked and Modal closed');
    }
